...
def write_staged(output: Path, files: dict[str, bytes]) -> None:
    output = _resolve_safe(output, label="output")
    output.parent.mkdir(parents=True, exist_ok=True)
    if output == output.parent or (output / "dataset.json").exists():
        raise ValueError(f"refusing unsafe output directory: {output}")
    if any(not _valid_path(path) for path in files):
        raise ValueError("generated release contains an unsafe path")
    if output.exists():
        ...
        for path in output.rglob("*"):
            ...
            if path.is_file():
                ...
                raise ValueError(f"refusing to replace special output entry: {path}")
        try:
            previous = load_json(output / "manifest.json")
            if (
                previous.get("dataset") != "mojilex"
                or previous.get("manifest_version") != MANIFEST_VERSION
            ):
                raise DataError("wrong manifest identity")
            allowed = {"manifest.json", "SHA256SUMS"}
            allowed.update(item["path"] for item in previous["artifacts"])
            allowed.update(
                item["path"]
            ...
            raise ValueError(f"refusing unrecognized non-build directory: {output}") from exc
        if not actual_files <= allowed:
...
def build_index(
    ...
    *,
    ...
    if snapshot_id is None or source_date_epoch is None:
        raise ValueError("snapshot_id and source_date_epoch are required immutable release inputs")
    ...
    for field, (filename, root_scope) in PROFILE_FILES.items():
        ...
        # Profile selectors are SHA-256(JCS(full profile)). Authority profile
        # files are themselves stored as those canonical JCS bytes so the
        # exact-content resource, source_path, and selector digest coincide.
        ...
        if field in DELEGATED_PROFILE_TYPES:
            ...
            if body.get("contract_schema_sha256") != sha256_file(contract_path):
    ...
    build_base = {
        "tool": "mojilex-data-builder",
        "tool_version": "0.2.0",
        "tool_repository": dataset["canonical_repository"],
        "tool_commit": revision,
        "dependency_lock_sha256": sha256_file(root / "requirements-dev.txt"),
        "distribution_profile": profiles["distribution"]["id"],
